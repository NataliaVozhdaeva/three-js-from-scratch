...
function getParticleSystem(params) {
  ...
  function _AddParticles(timeElapsed) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const spawnX = Math.sin(phi) * Math.cos(theta);
      const spawnY = Math.sin(phi) * Math.sin(theta);
      const spawnZ = Math.cos(phi);
      const normal = new THREE.Vector3(spawnX, spawnY, spawnZ);

      _particles.push({
        position: new THREE.Vector3(spawnX, spawnY, spawnZ).add(emitter.position),
